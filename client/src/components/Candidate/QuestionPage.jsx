...
const handleSession = () => {
  let allQuestions = [];
  for (let index = 0; index < gbTestId.length; index++) {
    const questionsString = sessionStorage.getItem(gbTestId[index]);
    if (questionsString) {
      const questionsArray = JSON.parse(questionsString);
      const transformedQuestions = transformQuestions(questionsArray);
      allQuestions = allQuestions.concat(transformedQuestions);
    }
  }
}
const transformQuestions = (quest) => {
  return quest.map(que => ({
  ...
  return (
    <div className="flex flex-col items-center p-8">
      {isSectionActive && <QuizTimer duration={quizDuration} onTimeUp={handleTimeUp} />}
      ...
      <div className="flex mt-6 space-x-4">
        <button
          className={`px-6 py-2 text-white font-semibold rounded ${isSectionActive ? 'bg-gray-500 hover:bg-gray-600' : 'bg-gray-300 cursor-not-allowed'}`}
          onClick={handleNext}
          disabled={!isSectionActive}
        >
          Next
