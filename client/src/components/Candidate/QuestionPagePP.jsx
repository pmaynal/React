    ...
    try {
      ...
      for (const testId of tIds) {
        const response = await axios.get(
          `http://127.0.0.1:8090/sharktest/question/test/${testId}`
        );
        ...
        // sessionStorage.setItem(testId, JSON.stringify(questionsData[testId]));
      }
...
//     try {
//       const response = await axios.get(
//         "http://127.0.0.1:8082/assessments/name/1"
//       );
//       setAssessmentName(response.data);
//       await getTests();
//     } catch (error) {
...
//     try {
//       const response = await axios.get(
//         "http://127.0.0.1:8082/assessments/testIds/1"
//       );
//       if (
//         response.data &&
...
//         await getQuestions(response.data);
//       } else {
//         setError("No test IDs found");
//         setIsLoading(false);
//       }
//     } catch (error) {
//       setError("Error fetching test IDs");
//       setIsLoading(false);
//     }
//   };
  ...
  const handleNext = () => {
    if (isSectionActive) {
      if (currentQuestionIndex < questions[testIds[currentSectionIndex]].length) {
        ...
        //     setIsSubmitActive(false);
        //   }
          setIsSectionActive(true); // Reset section activation
        } 
        else {
            setIsSubmitActive(false);
        //   setIsSectionActive(false);
